...
class Loan_model extends CI_Model {
    ...
    public function saveLoan() {
        ...
        $this->guarantorFileUpload();
        $this->collateralFileUpload();
        $this->addForm();
        $this->saveLoanGuarantor();
        $this->saveLoanCollateral();
    ...
    public function updateLoan() {
       ...
       return $this->db->trans_complete();
        ...
        public function guarantorFileUpload() {
                $grRowCnt = $this->input->post('grRowCnt');
                $files = array();
                $files = $_FILES;
                
                $cpt = count($_FILES ['files0'] ['name']);
                
                 $this->load->library('upload');
                 
                 for($j=0; $j<$grRowCnt; $j++)
                    {           
                 for($i=0; $i<$cpt; $i++)
                    {           
                        $_FILES['files'.$j]['name']= $files['files'.$j]['name'][$i];
                        $_FILES['files'.$j]['type']= $files['files'.$j]['type'][$i];
                        $_FILES['files'.$j]['tmp_name']= $files['files'.$j]['tmp_name'][$i];
                        $_FILES['files'.$j]['error']= $files['files'.$j]['error'][$i];
                        $_FILES['files'.$j]['size']= $files['files'.$j]['size'][$i];    

                        $this->upload->initialize($this->set_upload_options($j));
                        if (!$this->upload->do_upload('files'.$j)) {
        ...
        public function collateralFileUpload() {
                $clRowCnt = $this->input->post('clRowCnt');
                $files = array();
                $files = $_FILES;
                
                $cpt = count($_FILES ['filesColl0'] ['name']);
                
                 $this->load->library('upload');
                 
                 for($j=0; $j<$clRowCnt; $j++)
                    {           
                 for($i=0; $i<$cpt; $i++)
                    {           
                        $_FILES['filesColl'.$j]['name']= $files['filesColl'.$j]['name'][$i];
                        $_FILES['filesColl'.$j]['type']= $files['filesColl'.$j]['type'][$i];
                        $_FILES['filesColl'.$j]['tmp_name']= $files['filesColl'.$j]['tmp_name'][$i];
                        $_FILES['filesColl'.$j]['error']= $files['filesColl'.$j]['error'][$i];
                        $_FILES['filesColl'.$j]['size']= $files['filesColl'.$j]['size'][$i];    

                        $this->upload->initialize($this->set_upload_options2($j));
                        if (!$this->upload->do_upload('filesColl'.$j)) {
            $data['imageError'] =  $this->upload->display_errors();
            print_r($data['imageError']);
        }
                    }
        ...
        private function set_upload_options($i)
            {   
             ...
             $loan_id = $this->input->post("serialNo");
            $file_path = "./uploads/gurantor_file/" . $loan_id . '/';
            $file_path2 = "uploads/gurantor_file/" . $loan_id . '/G'.$i. '/';
            
            
            if (isset($_FILES['files0'])) {

                if (!is_dir('uploads/gurantor_file/' . $loan_id.'/G'.$i)) {
                    mkdir('./uploads/gurantor_file/' . $loan_id .'/G'.$i, 0777, TRUE);
                }
                
                $config = array();
                $config['upload_path'] = 'uploads/gurantor_file/'. $loan_id.'/G'.$i;
        ...
        private function set_upload_options2($j)
            {   
            
             $loan_id = $this->input->post("serialNo");
            $file_path = "./uploads/collateral_file/" . $loan_id . '/';
            $file_path2 = "uploads/collateral_file/" . $loan_id . '/C'.$j. '/';
           
            if (isset($_FILES['filesColl0'])) {

                if (!is_dir('uploads/collateral_file/' . $loan_id. '/C'.$j)) {
                    mkdir('./uploads/collateral_file/' . $loan_id. '/C'.$j, 0777, TRUE);
                }
                
                $config = array();
                $config['upload_path'] = 'uploads/collateral_file/'. $loan_id. '/C'.$j;
                $config['allowed_types'] = 'gif|jpg|png|jpeg|pdf|doc|xml';
                $config['max_size']      = '10000KB';
                $config['overwrite']     = FALSE;
    ...
    public function addForm() {
        ...
        $data = array(
            ...
            'id_loan_product' => $this->input->post('id_loan_product'),
            'disbursed_by' => $this->input->post('disbursed_by'),
            'created_date' => date('Y-m-d'),
            'release_date' => $this->input->post('release_date'),
            'borrower_id' => $this->input->post('borrower_id'),
            'principle_ammount' => $this->input->post('principle_ammount'),
...
//        print_r($data);
//            exit();
        $this->db->insert('loan_tbl', $data);
        $this->h_id = $this->db->insert_id();
       
    }

    public function saveLoanGuarantor() {
     $grRowCnt = $this->input->post('grRowCnt');
       
     if($grRowCnt){
         for($i=0; $i<$grRowCnt; $i++){
             if($this->input->post('guarantor'.$i)!=''){
        $data = array(
            'h_id' => $this->h_id,
            'guarantor' => $this->input->post('guarantor'.$i),
            'description' => $this->input->post('discription'.$i),
            'file_path' => 'G'.$i,
        );
         $this->db->insert('loan_guarantor', $data);
         }
     }
     }
    }

    public function saveLoanCollateral() {
     $clRowCnt = $this->input->post('clRowCnt');
       
     if($clRowCnt){
         for($i=0; $i<$clRowCnt; $i++){
             if($this->input->post('collateral'.$i)!=''){
        $data = array(
            'h_id' => $this->h_id,
            'collateral' => $this->input->post('collateral'.$i),
            'description' => $this->input->post('discription'.$i),
            'file_path' => 'C'.$i,
        );
...
//            exit();
    ...
    public function checkSerial($serialNo,$form){
        ...
        $res    =   $this->getLoanSerial($serialNo);
...
//            echo $serialNo;exit();
            ...
            if (isset($chk_box)) {
                ...
                if ($sql->num_rows() > 0) {
//                    echo '<pre>';
                    $sql = $this->db->query("UPDATE  loan_tbl SET approved='A' WHERE loan_id='" . $serialNo . "'");
                    $this->saveLoanSchedule($serialNo);//save loan schedule for repayment.
//                    print_r($sql->result());
                } 
            } 
        }
     ...
     public function saveLoanSchedule($serialNo) {
         $interest = 0;
         $due = 0;
         $pending_due = 0;
         $principal_bal = 0;
...
//            echo $serialNo;exit();
            ...
            if (isset($chk_box)) {
                ...
                if ($sql->num_rows() > 0) {
//                    echo '<pre>';
                    $sql = $this->db->query("UPDATE  Loan_tbl SET approved='R' WHERE loan_id='" . $serialNo . "'");
//                    print_r($sql->result());
                } 
            } 
        }
    ...
    public function getLoanSerial($serial){
        $serial =   $this->db->escape($serial);
        return $this->db->query("SELECT * FROM loan_tbl WHERE serialNo=$serial")->result();
    }
    ...
    public function loadPendingLoanList() {
        return $res = $this->db->query("select lt.*,bt.`name`,lp.`loan_product_name` from `loan_tbl` lt left join `loan_product` lp on lt.`id_loan_product`=lp.`id_loan_product`
left join `borrower` bt on lt.`borrower_id`=bt.`id_borrower`")->result();
    }

    public function loadLoansForApproval() {
...
left join `borrower` bt on lt.`borrower_id`=bt.`id_borrower` where lt.approved='P'";
//            echo $res;exit();
...
WHERE lt.loan_id =$loan_id")->result();
...
//                        ->where('status = 1')
    ...
    public function loadGuarantors() {
        return $res = $this->db->select('*')
                        ->from('guarantor')
//                        ->where('status = 1')
                        ->get()->result();
    ...
    public function loadLoanProduct() {
        return $res = $this->db->select('*')
                        ->from('loan_product')
//                        ->where('status = 1')
                        ->get()->result();
...
//  ---------------------------------- 2018-05-15 ---------------------------------
    // get Loans for update
    public function getLoans($id){
        return $this->db->query("select * from `loan_tbl` where `loan_id`=$id")->result();
    }
